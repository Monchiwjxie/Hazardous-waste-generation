...
# 模型构建 --------------------------------------------------------------------
#### 数据导入—训练数据及测试用例 ####
HWsum_3982_SMOGN = read.csv("HWsum_3982_smogn.csv")
data_test = read.csv("HWsum_3982_test.csv")

#### 选择变量 ####
data_model_train = HWsum_3982_SMOGN %>% 
  select(staff,
...
write.csv(truth_response, "HWsum_3982_tru_res.csv")
...
Performance_parameters = data.table(result_rsq, result_rmse, result_mae,result_mape,result_sse,
                                    model_param)
write.csv(Performance_parameters, "HWsum_3982_Performance_parameters.csv")