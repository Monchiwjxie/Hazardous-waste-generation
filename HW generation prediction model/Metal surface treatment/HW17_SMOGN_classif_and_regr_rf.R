...
# 数据导入 --------------------------------------------------------------------
#### 分类训练集及变量筛选 ####
data_clas_train = read.csv("one_typical_combined_classification_train.csv") %>% 
  select(industry,staff,
         ...
         HW17logic)
#### 回归训练集及变量筛选 ####
data_regr_train = read.csv("one_typical_combined_Regression_train.csv")%>% 
  select(industry,staff,
         process_1,process_2,process_3,process_4,process_5,
         process_6,process_7,process_8,process_9,process_10,
         process_11,
         ...
         HW17)
#### 测试集导入 ####
data_test = read.csv("HW17_combined_test.csv")
