...
def create_Classifer(input_dim: int, mlp_list: list, output_dim: int,
    ...
    """
    Initialize a Tabular neural network for classification

    Args:
        input_dim(int): Number of input features
        mlp_list(list): Optional MLP framework
        output_dim(int): Final output dimension of TabularNN
        cat_idxs(list): Categorical features index
        cat_dims(list): Output dimension of each categorical feature embedding layer
        cat_emb_dim(int): Output dimension of all of categorical feature embedding layers
        dense_idxs(list): Continuous features index
        dropout_list: Dropout rate of each layer
        lr: Learning rate

    Returns:
        model: An initialized TabularNN classifier
    """
...
def create_NN(input_dim: int, mlp_list: list, output_dim: int,
              ...
              dropout_list: list = None, lr: float = None):
    """
    Initialize a Tabular neural network for regression
    ...
    Returns:
        model: An initialized TabularNN regressor
    """
    all_inputs = Input(shape=(input_dim,))
    # Process dense features
    if dense_idxs is not None and len(dense_idxs) != 0:
...
def simple_train(train_data: Tuple[np.array, np.array], validation_data: Tuple[np.array, np.array],
    ...
    """
    Simple train function for TabularNN classifier or regressor

    Args:
        train_data(tuple): Train_X and Train_y
        validation_data(tuple): Test_X and Test_y
        input_dim(int): Input feature dimension
        config(Config): Train configs
        cat_idxs(list): Categorical features index
        cat_dims(list): Output dimension of each categorical feature embedding layer
        cat_emb_dim(int): Output dimension of all of categorical feature embedding layers
        dense_idxs(list): Continuous features index
        option_idx(list): Chose hyperparameters list
        type(str): regr or class

    Returns:
        history: Model training logging
        model: Trained TabularNN
    """
...
class GSCV:
    """
    A grid search cross validation framework for TabularNN, which can find a set of best hyperparameters

    Args:
        train_data(tuple): Train_X and Train_y
        params_list(list): Optional hyperparameters
        cat_idxs(list): Categorical features index
        cat_dims(list): Output dimension of each categorical feature embedding layer
        dense_idxs(list): Continuous features index
        task(str): regr or class
    """

    def __init__(self, train_data: Tuple[np.array, np.array],
                 params_list: list,cat_idxs: list, cat_dims:list,
                 dense_idxs: list, task:str='regr'):
    ...
    def option_params(self)->None:
        """
        Simply use for-loop to build a optional hyperparameters list
    ...
    def run(self)->list:
        """
        Run：build all optional hyperparameters combination -> run CV to find the best combination
...
if __name__ == "__main__":
    ...
    _,model_class = simple_train(train_data=(X_train, y_train_cp), validation_data=(X_test, y_target_cp),
                                 input_dim=X_train.shape[1], config=config_class, cat_idxs=cat_idxs,
                                 cat_dims=cat_dims, dense_idxs=dense_idxs,option_idx= class_idx, type="class")

    _,model_regr = simple_train(train_data=(X_regr, y_regr), validation_data=(X_test, y_test),
                                input_dim=X_train.shape[1], config=config_regr, cat_idxs=cat_idxs,
                                cat_dims=cat_dims,option_idx= regr_idx, dense_idxs=dense_idxs)
