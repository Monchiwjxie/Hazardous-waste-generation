...
for industry in ['combined', 2614, 2631, 2669, 3130, 3340, 3360, 3982, 4417, 4419]:
    ...
    '''
    df_train_data = pd.read_csv(f'data/combined_newdata.csv')
    if industry != 'combined':
        df_train_data = df_train_data.loc[df_train_data.industry == industry]
        df_train_data = df_train_data.iloc[:, 1:]
    if HWtype == 'HWsum':
        df_train_data = df_train_data.drop(columns = 'HW17')
    else:
        df_train_data = df_train_data.drop(columns='HWsum')
    '''
    df_train_data = df_train_data.iloc[:,1:]
