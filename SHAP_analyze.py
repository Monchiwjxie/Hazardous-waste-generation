...
import shap
import pandas as pd
from shap.plots.colors._colors import red_blue, red_blue_no_bounds, red_blue_transparent, red_blue_circle, red_transparent_blue
from shap.plots.colors._colors import transparent_blue, transparent_red, light_red_rgb, light_blue_rgb, gray_rgb, red_rgb, blue_rgb
...
#X and target Y setting
predictors =dataset.columns[:-1]
X_train = dataset[predictors]
y_train = dataset.iloc[:,-1].values

#RF model train
regressor = RandomForestRegressor(n_estimators = 800, max_depth = 11, min_samples_leaf = 6)
regressor.fit(X_train, y_train)

#creat explainer to caculate the SHAP value
...
#         cb.ax.set_aspect((bbox.height - 0.9) * 20)
    ...
    if plot_type == "bar":
        ...
        pl.xlabel(labels['VALUE'], fontsize=16)
    pl.tight_layout()
    if show:
        pl.show()

#Output the SHAP summary plots
fig = pl.figure()
summary_legacy(shap_values,X_train,plot_type="dot")
fig.savefig(r"./SHAP_plot.pdf",bbox_inches = 'tight',pad_inches = 0.1,dpi = 1000)

#Exporting SHAP values and eigenvalue
RF_shap_values=pd.DataFrame(shap_values)
RF_shap_values.head()
RF_shap_values = pd.DataFrame(RF_shap_values)
RF_shap_values.to_csv('./shap_value.csv')
data_with_name = pd.DataFrame(X_train)
data_with_name.to_csv('./frature_value.csv')